...
export function onMapSelect2(selection: MapSelection, erection: BuildType): void {
            ...
            if (selection.end) {
                if (selection.start.x == selection.end.x) {
                    for (let yStep = selection.start.y; yStep <= selection.end.y; yStep += mapTileSize) {
                        tile = map.getTile(selection.start.x, yStep)
                        ...
                        tile.elements.forEach( element => {
                            if (element.type == "surface") {
                                ...
                                debug(element.clearanceHeight.toString())
                            }
                        } )
                    }  
